...
resource "aws_security_group_rule" "securitygroup-ingress-tcp-from-cidrs" {
  count = "${length(var.security_group_ports_tcp) *
             length(var.security_group_cidrs)}"
  security_group_id = "${aws_security_group.securitygroup.id}"
  type = "ingress"
  protocol = "tcp"
  from_port = "${element(var.security_group_ports_tcp,
                         count.index / length(var.security_group_ports_tcp))}"
  to_port = "${element(var.security_group_ports_tcp,
                       count.index / length(var.security_group_ports_tcp))}"
  cidr_blocks = [
    "${element(var.security_group_cidrs,
               count.index % length(var.security_group_ports_tcp))}"
  ]
...
resource "aws_security_group_rule" "securitygroup-ingress-tcp-from-ids" {
  ...
  source_security_group_id =
    "${element(var.security_group_ids,
               count.index % length(var.security_group_ports_tcp))}"
...
resource "aws_security_group_rule" "securitygroup-ingress-udp-from-cidrs" {
  count = "${length(var.security_group_ports_udp) *
             length(var.security_group_cidrs)}"
  security_group_id = "${aws_security_group.securitygroup.id}"
  type = "ingress"
  protocol = "udp"
  from_port = "${element(var.security_group_ports_udp,
                         count.index / length(var.security_group_ports_udp))}"
  to_port = "${element(var.security_group_ports_udp,
                       count.index / length(var.security_group_ports_udp))}"
  cidr_blocks = [
    "${element(var.security_group_cidrs, count.index %
               length(var.security_group_ports_udp))}"
  ]
