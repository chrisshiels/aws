...
describe vpc('vpc-dev') do
  ...
  it { should have_route_table('rtb-dev-data-1a') }
  it { should have_route_table('rtb-dev-data-1b') }
end
...

{
  ...
  describe subnet("sn-dev-data-#{az.split('-').last()}") do
    it { should exist }
    it { should be_available }
    it { should belong_to_vpc('vpc-dev') }
    its(:cidr_block) { should eq cidr }
...
describe route_table('rtb-dev-data-1a') do
  it { should exist }
  it { should belong_to_vpc('vpc-dev') }
  it { should have_route('10.0.0.0/16').target(gateway: 'local') }
  it { should have_subnet('sn-dev-data-1a') }
  its('routes.last.nat_gateway_id') { should match /^nat-/ }
end


describe route_table('rtb-dev-data-1b') do
  it { should exist }
  it { should belong_to_vpc('vpc-dev') }
  it { should have_route('10.0.0.0/16').target(gateway: 'local') }
  it { should have_subnet('sn-dev-data-1b') }
  its('routes.last.nat_gateway_id') { should match /^nat-/ }
end
...
describe security_group('sg-dev-app-db') do
  it { should exist }
  it { should belong_to_vpc('vpc-dev') }
  its(:outbound) { should be_opened.protocol('all').for('0.0.0.0/0') }
...
describe rds('db-dev-app') do
  ...
  it { should have_security_group('sg-dev-app-db') }
  it { should have_security_group('sg-dev-all') }
  it { should belong_to_db_subnet_group('dbsng-dev-app') }
  it { should belong_to_vpc('vpc-dev') }
  ...
  it { should have_tag('Name').value('db-dev-app') }
